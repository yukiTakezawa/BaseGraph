...
class QgDsgdmOptimizer(Optimizer):
    def __init__(self, params, node_id: int, graph, local_step, lr=1e-5, beta=0.9, device="cuda"):
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                group["hat_momentum"].append(torch.zeros_like(p, device=self.device))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            #for p, momentum, hat_momentum in zip(group['params'], group["momentum"], group["hat_momentum"]):
            for p, hat_momentum in zip(group['params'], group["hat_momentum"]):
                momentum = p.grad + beta * hat_momentum
    ...
    def average_param(self, recieved_params, neighbors): 
        ...
        for group in self.param_groups:
            ...
            for i, (p, hat_momentum) in enumerate(zip(group["params"], group["hat_momentum"])):
                d = p.data * 1.0
                ...
                d = (d - p) / beta
                hat_momentum.data = beta * hat_momentum + (1 - beta) * d
