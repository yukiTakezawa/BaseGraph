...
class VisHyperHyperCube(DynamicGraph):
            ...
            for i_idx in range(len(node_list)):
                for nk in range(1, factors_list[k]):
                    ...
                    if b[i] < factors_list[k]-1 and b[j] < factors_list[k]-1:
                        ...
                        w[i, j], w[j, i] = -1, -1 #1/factors_list[k], 1/factors_list[k]
                        w[i, i], w[j, j] = -1, -1 #1/factors_list[k], 1/factors_list[k]
...
class VisSimpleAdicGraph(DynamicGraph):
    ...
    def construct(self):
        ...
        if self.n_nodes == 1:
            ...
            return VisHyperHyperCube(self.n_nodes, max_degree=self.max_degree).w_list

        # construct k-peer VisHyperHyperCube
        hyperhyper_cubes = [VisHyperHyperCube(len(node_list_list[i]), max_degree=self.max_degree) for i in range(L)]
        hyperhyper_cubes2 = [VisHyperHyperCube(len(node_list_list_list[i][0]), max_degree=self.max_degree) for i in range(L)]
                ...
                if m < max_length_of_hyper:
                    ...
                    for i in node_list_list[l]:
                        ...
                        for k in range(a_l):
                            ...
                            w[i, j] = -2 #n_nodes_list[m - max_length_of_hyper] / sum(n_nodes_list[m - max_length_of_hyper:]) / a_l
                            w[j, i] = -2 #n_nodes_list[m - max_length_of_hyper] / sum(n_nodes_list[m - max_length_of_hyper:]) / a_l

                            w[j, j] = -2 #1 - w[i, j]
                        w[i, i] = -2 #1 - n_nodes_list[m - max_length_of_hyper] / sum(n_nodes_list[m - max_length_of_hyper:])
                    ...
                    while len(all_isolated_nodes) > 1 and self.inner_edges:
                        ...
                        for i in sampled_nodes:
                            for j in sampled_nodes:
                                w[i, j] = -3 #1 / len(sampled_nodes)
                                w[j, i] = -3 #1 / len(sampled_nodes)
                                w[i, i] = -3 #1 / len(sampled_nodes)
                                w[j, j] = -3 #1 / len(sampled_nodes)

                else:
                    if n_nodes_list[l] < self.max_degree+1:
                        length = len(hyperhyper_cubes[l].w_list)
                        w += self.extend(hyperhyper_cubes[l].w_list[int(b[l] % length)], node_list_list[l]) * 5.0
                    else:
                        a_l = len(node_list_list_list[l])

                        for k in range(a_l):
                            length = len(hyperhyper_cubes2[l].w_list)
                            w += self.extend(hyperhyper_cubes2[l].w_list[int(b[l] % length)], node_list_list_list[l][k]) * 4.0
