...
class D2Optimizer(Optimizer):
    def __init__(self, params, node_id: int, graph, local_step, lr=1e-5, beta=0.9, device="cuda"):
        ...
        super(D2Optimizer, self).__init__(params, defaults)
        ...
        for group in self.param_groups:
            ...
            for p in group["params"]:
                ...
                group["old_param"].append(torch.zeros_like(p, device=self.device))
                group["old_grad"].append(torch.zeros_like(p, device=self.device))
    ...
    def step(self, closure=None):
        ...
        for group in self.param_groups:
            ...
            for p, old_p, old_grad in zip(group['params'], group["old_param"], group["old_grad"]):
                p.data = 2*p - old_p - lr*p.grad - old_grad

                old_p.data = p
                old_grad.data = lr*p.grad
